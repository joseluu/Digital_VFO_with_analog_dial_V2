...
public:
  LGFX(void)
  {
    { // // Configuring the SPI bus
      ...
      cfg.freq_write = 40000000;    // SPI clock at transmission (rounded to 80MHz, 80MHz divided by an integer)
      ...
      cfg.pin_miso = 12;            // Set the MISO pin number for SPI (-1 = disable)
      cfg.pin_dc   = 32;            // Set the D/C pin number for SPI (-1 = disable)
    ...
    { // Sets the display panel control.
      ...
      cfg.pin_cs           =    27;  // Pin number to which CS is connected (-1 = disable)
      ...
      cfg.panel_width      =   240;// 128; // Actual displayable width
      cfg.panel_height     =   320;// 160; // Actual display height
